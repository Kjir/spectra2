...
comando, inizializzando le variabili con valori appropriato ed allocando alcuni
buffer. Inizializza poi un buffer circolare utilizzato per i dati di input, una
lista per i dati di output 

\subsection{Thread di output}
Poi avvia il thread di output, il quale controlla i dati presenti
in una lista, fornita all'avvio, dove verranno accodati i dati da scrivere in
uscita. Quando la lista \`e vuota, il thread si mette semplicemente in attesa,
altrimenti procede alla scrittura e ricomincia il proprio ciclo da capo.
\begin{figure}[htb]
	\begin{center}
